...
const SellerDashboard = () => {
  ...
  // Simple PDF Export fallback function
  const handleSimplePDFExport = useCallback(() => {
...
${analyticsData.topProducts?.slice(0, 5).map(product => 
  `- ${product.name}: ₱${product.revenue?.toLocaleString() || '0'} (${product.orders || 0} orders)`
).join('\n') || 'No products data available'}

CUSTOMER INSIGHTS:
- Total Customers: ${analyticsData.customerInsights?.totalCustomers || '0'}
- Repeat Customers: ${analyticsData.customerInsights?.repeatCustomers || '0'}
- Average Order Value: ₱${analyticsData.customerInsights?.averageOrderValue?.toLocaleString() || '0'}
- Customer Satisfaction: ${analyticsData.customerInsights?.customerSatisfaction?.toFixed(1) || '0.0'}%

INVENTORY METRICS:
- Low Stock Items: ${analyticsData.inventoryMetrics?.lowStockItems || '0'}
- Out of Stock Items: ${analyticsData.inventoryMetrics?.outOfStockItems || '0'}
- Total Inventory Value: ₱${analyticsData.inventoryMetrics?.totalInventoryValue?.toLocaleString() || '0'}
- Inventory Turnover: ${analyticsData.inventoryMetrics?.inventoryTurnover?.toFixed(1) || '0.0'}x

Generated by Green Marketplace Analytics
    `;
    ...
    a.href = url;
    a.download = `seller-analytics-${user.name || user.email}-${analyticsTimeframe}-${new Date().toISOString().split('T')[0]}.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    
    toast.success('Analytics report downloaded as text file!');
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    try {
      ...
      console.log('Falling back to simple text export...');
      
      // Fallback to simple text export
      handleSimplePDFExport();
    } finally {
                    ...
                    <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
                      ...
                      <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                        ...
                        >
                          ...
                          {pdfExportLoading ? 'Generating PDF...' : '📄 Export PDF Report'}
                        </Button>
